    ...
    <div class="section kce-ship-table">
        <div class="filters">
            <div class="filter type">
                ...
                <span v-for="f in order" :key="'filter_'+f" @click="toggleTypeFilter(f)" :class="{selected:typeSelected.indexOf(f)!==-1}">{{f}}</span>
        ...
        <div class="shipTable">
            <div class="shipRow header">
                ...
                <div class="noHover slots" @click="setSort('slots')">Slots</div>
            </div>
            <ship-table-row v-for="ship in finilShipList" :class="{hidden:!ship.show,odd:ship.odd}" :ship="ship" :key="`s`+ship.id"></ship-table-row>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            setSort(type){
                if(this.sortBy === type){
                    this.sortByInverse = this.sortByInverse*-1;
                }else{
                    this.sortBy = type;
